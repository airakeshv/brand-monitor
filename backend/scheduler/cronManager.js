...
// convert HH:MM in user timezone to a UTC cron expression
function toCronExpression(timeHHMM, timezone) {
  const [hours, minutes] = timeHHMM.split(':').map(Number);
  const now = new Date();
  const pad = n => String(n).padStart(2, '0');
  // Build a wall-clock string (no offset) so fromZonedTime treats it as the user's timezone
  ...
  // Use UTC getters — the cron is run with { timezone: 'UTC' }
...
// start the daily cron schedule based on current settings
export function scheduleDigest() {
  ...
  const cronExpr = toCronExpression(settings.delivery_time, timezone);

  activeTask = cron.schedule(cronExpr, () => deliverDigest(getSettings()), { timezone: 'UTC' });
  console.log(`Digest scheduled at ${settings.delivery_time} ${timezone} → cron: ${cronExpr}`);
}
